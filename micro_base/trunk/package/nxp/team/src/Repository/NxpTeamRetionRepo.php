...
class NxpTeamRetionRepo extends Repository
{
    ...
    public function getMyPromotion($request)
    {
        ...
        if($ret['data']){
            foreach($ret['data'] as $k=>$v){
                $ret['data'][$k]->login_name = substr($v->login_name, 0, 3).'****'.substr($v->login_name, 8);
            }
        }
        return $ret['data'];
    }
    ...
    //查询团队
    public function getMyPromotionAll($request)
    {
        if (!$request['pageSize']) {
            $request['pageSize'] = 10;
        }
        $ret = optional(DB::table('team_relation as t1')
            ...
            ->where('t2.user_tariff_code', '!=', 'P1101')
            ->orderBy('t2.user_tariff_code', 'DESC')
            ->orderBy('t2.create_time', 'DESC')
            ->paginate($request['pageSize']))
            ->toArray();
...
//                    ->select('id','user_id')
    ...
    public function getDirectUserByUserId($request){
        $ret = DB::table('team_relation as t0')
            ->select('t1.user_id','t1.user_name','t1.user_tariff_code')
            ->leftJoin('comm_user_info as t1',function ($join) use ($request){
                $join->on('t0.user_id', '=', 't1.user_id');
            })
            ->where('t0.parent1',$request['user_id'])
            ->where('t1.user_tariff_code' ,'>=', $request['user_tariff_code'])
            ->count();
        return $ret;
    }
    ...
    public function getLevelUserByUserId($request){
        $ret = DB::table('team_relation as t0')
            ...
            ->where('t0.team_user_level', 'like', '%'.$request['user_id'].'%')
            ->where('t1.user_tariff_code' ,'>=', $request['user_tariff_code'])
            ->count();
        return $ret;
    }
