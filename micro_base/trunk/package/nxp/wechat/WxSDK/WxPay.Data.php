...
class WxPayDataBase
{
	...
	**/
	...
	public function MakeSign()
	{
		...
		//签名步骤二：在string后加入KEY
        if ($this->values['trade_type'] == 'JSAPI') {

            $ret = \Micro\Common\Base\Base::code('wx_pay_config.jsapi');

            $string = $string . "&key=".$ret['property2'];
            \Illuminate\Support\Facades\Log::info('JSAPI'.$string);
        } else {
            $ret = \Micro\Common\Base\Base::code('wx_pay_config.key');

            $string = $string . "&key=".$ret['property2'];
            \Illuminate\Support\Facades\Log::info('KEY'.$string);
        }
		...
		\Illuminate\Support\Facades\Log::info('-------第一次签名-------------'.$result);
		return $result;
	}
