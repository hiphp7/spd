...
class Query{
    ...
    public function getMyPromotionById($request){
        ...
        foreach ($ret as $key => $value){
            $ret[$key]->rank = projectConfig('const_user.'.$value->user_tariff_code . '.code');
            $ret[$key]->level_name = projectConfig('const_user.'.$value->user_tariff_code . '.msg');
            $img = substr($value->headimgurl,-11);
            if($img !== 'headImg.png') $ret[$key]->headimgurl = R($value->headimgurl);
     ...
     * @params pageSize number 条数
     */
    public function getMyPromotionAllById($request){
        $ret = $this->team->getMyPromotionAll($request);
     ...
     * @desc 我的推广的人数
     */
    public function getMyPromotionAllCount($request){
        return $this->team->getMyPromotionAllCount($request);
    }
    ...
    */
    ...
    //根据用户ID,查询指定等级的直推用户
    public function getDirectUserByUserId($request){
        return $this->team->getDirectUserByUserId($request);
    }

    //根据用户ID,查询指定等级用户
    public function getLevelUserByUserId($request){
        return $this->team->getLevelUserByUserId($request);
    }
