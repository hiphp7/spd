...
class CommUserRepo extends Repository
{
    ...
    {
        $criteria = Criteria::create()->where('user_id',$user_id);
        return $this->cacheFirst(
            $user_id,
            config('cache.repo.middle'),
            $criteria,
            [
                'user_id','user_name','user_type','user_tariff_code',
    ...
    public function getUser($user_id){
        return optional($this->model
            ->select('id','user_id','user_name','user_type','status','login_name','level_id','merc_id','client_id','create_time',
                'user_tariff_code','crp_nm','crp_id_no','account_name','open_bank_name','cash_status','account_no','agent_id','regist_address','bank_line_name','bank_reserved_mobile','level_name','bank_code','headimgurl')
    ...
    public function getUserDataByLevel($level){
        return optional($this->model
            ->select('user_id','user_name','login_name')
            ->where('user_tariff_code',$level)
    ...
    //根据手机号获取用户资费编码
    public function getCodeByTel($tel,$projectCode)
    {
        $ret = optional($this->model
